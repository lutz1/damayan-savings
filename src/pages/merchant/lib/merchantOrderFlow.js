...
const ALIASES = {
  ...
  assigned: MERCHANT_ORDER_STATUS.PREPARING,
...
export const isMerchantOrderPreparing = (status) => {
  const normalized = normalizeMerchantOrderStatus(status);
  return normalized === MERCHANT_ORDER_STATUS.ACCEPTED || 
         normalized === MERCHANT_ORDER_STATUS.PREPARING;
};
...
export const isMerchantOrderInDelivery = (status) => {
  const normalized = normalizeMerchantOrderStatus(status);
  return normalized === MERCHANT_ORDER_STATUS.ORDER_PICKED_UP || 
         normalized === MERCHANT_ORDER_STATUS.IN_DELIVERY;
};
...
export const isMerchantOrderTerminal = (status) => {
  const normalized = normalizeMerchantOrderStatus(status);
  return normalized === MERCHANT_ORDER_STATUS.DELIVERED || 
         normalized === MERCHANT_ORDER_STATUS.CANCELLED;
};
