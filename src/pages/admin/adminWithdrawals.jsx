...
const AdminWithdrawals = () => {
  ...
  const handleAction = async (status) => {
    ...
    try {
      ...
      await updateDoc(withdrawalRef, { status: status.toLowerCase(), reviewedAt: new Date(), remarks });
      ...
      <Box component="main" sx={{ flexGrow: 1, p: isMobile ? 1 : 3, mt: 8, color: "white", zIndex: 1, width: "100%", overflowX: "hidden" }}>
        ...
        <Card sx={{ background: "rgba(255,255,255,0.12)", backdropFilter: "blur(12px)", borderRadius: "16px", mb: 3, width: "100%", overflowX: "auto" }}>
          <CardContent sx={{ textAlign: isMobile ? "center" : "left" }}>
            ...
            <Typography sx={{ mt: 1 }}>Total Approved Withdrawals: <b>₱{salesData.total.toFixed(2)}</b></Typography>
            <Typography sx={{ mt: 1 }}>Revenue (5% charge): <b>₱{salesData.revenue.toFixed(2)}</b></Typography>
          </CardContent>
        </Card>
        ...
        <Card sx={{ background: "rgba(255,255,255,0.12)", backdropFilter: "blur(12px)", borderRadius: "16px", boxShadow: "0 6px 25px rgba(0,0,0,0.25)", width: "100%", overflowX: "auto" }}>
          <CardContent sx={{ p: 1 }}>
            {loading ? (
              ...
              <Typography align="center" sx={{ color: "rgba(255,255,255,0.7)", py: 3 }}>No withdrawals found.</Typography>
            ) : (
              <TableContainer sx={{ width: "100%", overflowX: "auto" }}>
                <Table size={isMobile ? "small" : "medium"}>
                  <TableHead>
                    <TableRow>
                      {["Name","Amount","Charge","Net Amount","Method","Status","Date","Actions"].map(head => (
                  ...
                  <TableBody>
                    {withdrawals.map(w => {
                      ...
                      return (
                        <motion.tr key={w.id} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.3 }}>
                          ...
                          <TableCell sx={{ color: "white", whiteSpace: "nowrap" }}>
                            {w.createdAt ? new Date(w.createdAt.seconds*1000).toLocaleString() : "—"}
                          </TableCell>
                          <TableCell>
                            {canApproveReject && status==="pending" ? (
                              <Box sx={{ display:"flex", gap:1, flexWrap:"wrap", justifyContent:"center" }}>
                                {w.qrUrl && <Button variant="outlined" color="info" size="small" onClick={()=>handleViewQR(w.qrUrl)}>View QR</Button>}
                                <Button variant="contained" color="success" size="small" onClick={()=>openDialog(w)}>Approve / Reject</Button>
                              </Box>
                            ) : (
        ...
        <Dialog open={!!selectedWithdrawal} onClose={closeDialog} fullWidth maxWidth="sm">
          ...
          <DialogContent>
            <Typography variant="body1" sx={{ mb: 2 }}>
              ...
              Charge: ₱{selectedWithdrawal?.charge || 0} | Net: ₱{selectedWithdrawal?.netAmount || selectedWithdrawal?.amount}
            </Typography>
        ...
        <Dialog open={!!qrImage} onClose={closeQRDialog} fullWidth maxWidth="xs">
          ...
          <DialogContent sx={{ textAlign:"center" }}>
            <img src={qrImage} alt="Withdrawal QR Proof" style={{ width:"100%", borderRadius:"12px", marginTop:"10px" }}/>
          </DialogContent>
          <DialogActions><Button onClick={closeQRDialog}>Close</Button></DialogActions>
        </Dialog>
      </Box>
    </Box>
  );
