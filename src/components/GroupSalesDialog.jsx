...
const GroupSalesDialog = ({ open, onClose, username, user }) => {
  ...
  const fetchNetworkUsers = async (rootUsername) => {
    ...
    while (queue.length) {
      ...
      snap.forEach((doc) => {
        const data = doc.data();
        if (data?.username) {
          users.push({ ...data, id: doc.id }); // store document ID
  ...
  useEffect(() => {
    ...
    const loadData = async () => {
      ...
      for (const u of allUsers) {
        const { total, hasEntry } = await fetchCapitalShareRecord(u.id); // use doc ID as userId
        if (hasEntry) {
          results.push({
            ...u,
